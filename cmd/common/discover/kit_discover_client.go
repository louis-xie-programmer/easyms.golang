...
import (
	...
	easylogger "github.com/louis-xie-programmer/easyms/pkg/logger"
	"strconv"
	"sync"
)
...
func (consulClient *KitDiscoverClient) Register(serviceName, instanceId, healthCheckUrl string, instanceHost string, instancePort int, meta map[string]string, logger *log.Logger) bool {
	...
	if err != nil {
		level.Error(*logger).Log(easylogger.LogEntry{
			Message: "Register consul server",
			Error:   err.Error(),
			Module:  "register consul server",
		})
		return false
	}
	level.Info(*logger).Log("info", easylogger.LogEntry{
...
func (consulClient *KitDiscoverClient) DeRegister(instanceId string, logger *log.Logger) bool {
	...
	if err != nil {
		level.Error(*logger).Log(easylogger.LogEntry{
			Message: "Deregister consul server",
			Error:   err.Error(),
			Module:  "Deregister consul server",
		})
		return false
	}
	level.Info(*logger).Log(easylogger.LogEntry{
		Message: "Deregister Service Success!",
		Module:  "Deregister consul server",
	})
...
func (consulClient *KitDiscoverClient) DiscoverServices(serviceName string, logger *log.Logger) []interface{} {
	...
	if err != nil {
		consulClient.instancesMap.Store(serviceName, []interface{}{})
		level.Error(*logger).Log(easylogger.LogEntry{
			Message: "Discover Consul Service Error!",
			Error:   err.Error(),
			Module:  "Discover Consul Service",
		})
		return nil
	}
